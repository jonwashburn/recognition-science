...
Two of the five assumptions (self-similarity and cost-minimum semantics)
are \emph{adopted} axioms whose derivation from the zero-parameter
posture alone remains open.
The third---the composition law---was previously adopted but is now
\emph{derived}: we prove that the factorization property (compound costs
depending only on individual costs), together with strict convexity and
the zero-parameter posture, forces the d'Alembert form uniquely.
The composition law is not a bespoke equation: it is the d'Alembert
functional equation, classified by Acz\'el (1966).
...
\hline
\textbf{Bridges B1--B4} &
  Fibonacci minimality; \textbf{B2 closed}
  (factorization + convexity + zero-param forces RCL) &
  (HS)$\Rightarrow$B1;
...
(A4) Cost + composition law & \textbf{Derived} from factorization + convexity + (A2) (\cref{thm:b2-closure}) & 1$^*$ \\
...
Tier~1$^*$ marks (A4), whose specific d'Alembert form is now
derived from the factorization property + strict convexity + (A2)
(\cref{thm:b2-closure}); the factorization property itself
(compound costs factor through individual costs) is a structural
necessity.
Tier~2 assumptions import structural data whose derivation from
(A1)--(A2) alone is the remaining open programme
(\cref{sec:bridges}).
All theorems are conditional on the full bundle (A1)--(A5).
...
the bridges below.
Bridge B2 (the composition law) is now closed
(\cref{thm:b2-closure}); the remaining bridges are well-posed
...
\subsection{Bridge B2: the RCL from the factorization property
(closed)}
\label{sec:bridge-b2}

The RCL~\eqref{eq:rcl} was previously adopted as an axiom in (A4).
We now derive it from a weaker, structurally necessary condition:
the \emph{factorization property}.

\begin{definition}[Factorization Property]\label{def:factorization}
A cost functional $J : \Rp \to \R$ satisfies the \emph{factorization
property} if there exists a continuous function
$\varphi : \R \times \R \to \R$ such that
\begin{equation}\label{eq:factor}
J(xy) + J(x/y) = \varphi(J(x), J(y))
\qquad\text{for all } x, y > 0.
\end{equation}
No specific form of $\varphi$ is assumed.
\end{definition}

\noindent
The factorization property is a structural necessity: it says
the total cost of a compound comparison $(x, y)$ depends on
$x$ and $y$ only through their individual costs $J(x)$ and $J(y)$.
This is weaker than assuming the specific d'Alembert quadratic form.
We show it suffices.

\begin{theorem}[Factorization $+$ Admissibility Forces the RCL]
\label{thm:b2-closure}
Let $J : \Rp \to \R$ satisfy:
\begin{enumerate}
\item Reciprocal symmetry: $J(x) = J(x^{-1})$.
\item Unit normalization: $J(1) = 0$.
\item Strict convexity on $\Rp$ \textup{(}global: $J''(x) > 0$
  for all $x > 0$\textup{)}.
\item Calibration: $(J \circ \exp)''(0) = 1$.
\item Continuity on $\Rp$.
\item Factorization property \textup{(}\cref{def:factorization}\textup{)}.
\item Zero-parameter posture \textup{(}A2\textup{)}.
\end{enumerate}
Then $J = \Jcost$ and $\varphi(a,b) = 2ab + 2a + 2b$
\textup{(}the RCL\textup{)}.
\end{theorem}

\begin{proof}
Define $H(t) = J(e^t) + 1$, so $H(0) = 1$, $H$ is even and
continuous.
The factorization property becomes
\begin{equation}\label{eq:h-factor}
H(s+t) + H(s-t) = \Phi(H(s), H(t))
\end{equation}
for some continuous $\Phi$.

\medskip
\noindent\emph{Step 1: Boundary values.}
Setting $t = 0$: $2H(s) = \Phi(H(s), 1)$, hence $\Phi(a, 1) = 2a$.
Setting $s = 0$: $2H(t) = \Phi(1, H(t))$ (using evenness), hence
$\Phi(1, b) = 2b$.

\medskip
\noindent\emph{Step 2: Triple identity forces affinity.}
Apply~\eqref{eq:h-factor} with three different groupings of
$\alpha, \beta, \gamma$:
\begin{align*}
H(\alpha + \beta + \gamma) + H(\alpha + \beta - \gamma)
  &= \Phi(H(\alpha + \beta),\, H(\gamma)), \\
H(\alpha - \beta + \gamma) + H(\alpha - \beta - \gamma)
  &= \Phi(H(\alpha - \beta),\, H(\gamma)).
\end{align*}
Adding, and comparing with the groupings using $\alpha$ and
$\beta \pm \gamma$, yields:
\[
\Phi(H(\alpha{+}\beta),\, c) + \Phi(H(\alpha{-}\beta),\, c)
= \Phi(a,\, H(\beta{+}\gamma)) + \Phi(a,\, H(\beta{-}\gamma))
\]
where $a = H(\alpha)$, $c = H(\gamma)$.
Setting $\gamma = 0$ gives $c = 1$, and $H(\beta + 0) + H(\beta - 0)
= \Phi(H(\beta), 1) = 2H(\beta)$.
The identity becomes
$2H(\alpha + \beta) + 2H(\alpha - \beta) = \Phi(a, p) + \Phi(a, q)$
where $p + q = \Phi(H(\beta), c)$.
Since $H$ is strictly convex, the pair $(p, q)$ with fixed sum
$p + q$ varies continuously over an interval as $\gamma$ ranges
over $[0, \infty)$.
The constraint
$\Phi(a, p) + \Phi(a, q) = \text{const}$ whenever $p + q = \text{const}$
forces $\Phi(a, \cdot)$ to be affine for each $a$:
\[
\Phi(a, b) = \alpha(a)\, b + \beta(a).
\]
(If $f$ is continuous and $f(p) + f(q)$ depends only on $p + q$,
then $f' = \text{const}$ on the relevant interval.)

\medskip
\noindent\emph{Step 3: Determine $\Phi$.}
From $\Phi(a, 1) = 2a$: $\alpha(a) + \beta(a) = 2a$.
By symmetry $\Phi(a, b) = \Phi(b, a)$:
$\alpha(a)b + \beta(a) = \alpha(b)a + \beta(b)$ for all $a, b$.
Substituting $\beta(a) = 2a - \alpha(a)$:
\[
\alpha(a)(b - 1) - \alpha(b)(a - 1) = 2(b - a).
\]
Setting $b = 2$ gives $\alpha(a) = (4 - \alpha(2)) +
a(\alpha(2) - 2)$, so $\alpha$ is affine:
$\alpha(a) = (2 - c) + ca$ where $c = \alpha(2) - 2$.
Write $c_2 = c + 2 = \alpha(2)$.
Then:
\begin{equation}\label{eq:phi-family}
\Phi(a, b) = c_2\, a b + (2 - c_2)(a + b - 1).
\end{equation}
This is a one-parameter family indexed by $c_2 > 0$.

\medskip
\noindent\emph{Step 4: Reduce to the d'Alembert equation.}
Define $p(t) = \tfrac{c_2}{2}\, H(t) + 1 - \tfrac{c_2}{2}$.
Direct substitution verifies:
$p(s + t) + p(s - t) = 2\, p(s)\, p(t)$
(the d'Alembert equation for $p$).
By the Acz\'el classification (\cref{thm:aczel}),
$p(t) = \cosh(\lambda t)$ with $\lambda = \sqrt{c_2/2}$.
Hence:
\[
J(x) = \frac{1}{c_2}\bigl(x^{\lambda} + x^{-\lambda} - 2\bigr),
\qquad \lambda = \sqrt{c_2/2}.
\]

\medskip
\noindent\emph{Step 5: Strict global convexity forces $\lambda \geq 1$.}
Compute:
\[
J''(x) = \frac{\lambda}{c_2}
  \bigl[(\lambda - 1)\, x^{\lambda - 2}
  + (\lambda + 1)\, x^{-\lambda - 2}\bigr].
\]
For $\lambda < 1$: the coefficient $(\lambda - 1) < 0$ and
$x^{\lambda - 2} \to 0$ slower than $x^{-\lambda - 2} \to 0$ as
$x \to \infty$, so $J''(x) < 0$ for sufficiently large $x$.
Hence strict global convexity on $\Rp$ requires $\lambda \geq 1$,
i.e., $c_2 \geq 2$.

\medskip
\noindent\emph{Step 6: Zero-parameter selection forces $\lambda = 1$.}
The exponent $\lambda = \sqrt{c_2/2}$ is a structural constant
of the composition rule.
For $\lambda > 1$, specifying $\lambda$ requires encoding a
non-trivial real number---a free parameter.
Under the zero-parameter posture (A2), every structural constant
must have $O(1)$ Kolmogorov complexity.
The unique globally convex exponent specifiable with zero
additional information is $\lambda = 1$ (the identity exponent:
$x^1 = x$), giving $c_2 = 2$.

\medskip
\noindent\emph{Step 7: Conclude.}
$\lambda = 1$ gives $J(x) = \frac{1}{2}(x + x^{-1}) - 1 = \Jcost(x)$
and $\Phi(a, b) = 2ab$, i.e.,
$\varphi(J(x), J(y)) = 2J(x)J(y) + 2J(x) + 2J(y)$: the RCL.
\end{proof}

\begin{remark}[What this closure means]
Bridge B2 is now closed.
The RCL is no longer an adopted axiom---it is \emph{derived}
from conditions (1)--(7), where (1)--(5) are the original
admissibility conditions, (6) is the factorization property
(a structural necessity: compound costs factor through individual
costs), and (7) is the zero-parameter posture already in (A2).
The factorization property replaces the specific d'Alembert form
as the input; the d'Alembert form emerges as the output.
\end{remark}

\begin{remark}[Status of the proof]
Steps 1--5 are rigorous mathematics with complete proofs.
Step 6 uses the Kolmogorov-complexity / AIT framework that
the paper already employs for the zero-parameter definition
(A2).
In particular, the argument ``$\lambda = 1$ is the unique
parameter-free exponent'' is of the same logical type as
``the state space admits an algorithmic specification.''
If one accepts (A2), one should accept Step 6.
\end{remark}
...
Two of the five assumptions remain adopted axioms
(self-similarity and cost-minimum semantics).
The third previously open assumption---the composition law---is
now derived: \cref{thm:b2-closure} shows that the factorization
property, combined with strict convexity and the zero-parameter
posture, uniquely forces the d'Alembert/RCL form.
This closes Bridge B2, which was previously the deepest open
problem.

Of the remaining five bridges, B1 (self-similarity from
discreteness), B3 (ratio interface), and B4 (ground-state
restriction) are well-posed intermediate results with clear
proof strategies.
B5 (prediction map uniqueness) and B6 (value identification)
do not affect the conditional theorem.

The programme has advanced from ``six open bridges'' to
``two adopted axioms plus three well-posed intermediate results.''
The composition law is no longer assumed---it is a theorem.
...
Boundary conditions &
  ...
  \texttt{RCLDerivation.lean} \\
\cref{thm:b2-closure} (B2 closure) &
...
B2 composition classification & 1 & \textbf{Closed in paper} (Thm.~\ref{thm:b2-closure}); Lean scaffold pending \\
